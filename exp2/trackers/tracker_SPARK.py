...
class Tracker(object):
    ...
    def track(self, image, attacker, prev_perts, weights, info: dict = None) -> dict:
        ...
        x_crop_adv, pert_true, prev_perts, weights, img_ = attacker.attack(self, image, prev_perts, weights, APTS,OPTICAL_FLOW,ADAPT)


        # normalize
        x_crop = x_crop_adv.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        x_crop[0] = tvisf.normalize(x_crop[0], self.mean, self.std, self.inplace)

        # track
        mask_flag = False
        outputs = self.net.track(x_crop) #, mask=mask_flag)
        ...
        return out, prev_perts, weights. x_crop_ori, x_crop_adv
