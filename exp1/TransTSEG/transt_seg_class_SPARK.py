...
class TRANST_SEG(object):
    ...
    def track(self, image, attacker, prev_perts, weights, gt):
        ...
        #SPARK Attack
        '''Apply the perturbation-UA'''
        APTS,OPTICAL_FLOW,ADAPT=False,False,False
        #x_crop, pert_sum, image 
        x_crop, pert_true, prev_perts, weights, img_ = attacker.attack(self, image, gt, prev_perts, weights, APTS,OPTICAL_FLOW,ADAPT)

        # normalize
        x_adv = x_crop.float().mul(1.0 / 255.0).clamp(0.0, 1.0)
        x_adv[0] = tvisf.normalize(x_adv[0], self.mean, self.std, self.inplace)

        # track
        mask_flag = True
        outputs = self.net.track_seg(x_adv, mask=mask_flag)
        ...
        return bbox, final_mask, prev_perts, weights
...
def run_vot_exp(name, window, penalty_k, mask_threshold, net_path, save_root, VIS=False):
    ...
    '''Build SPARK Attacker'''
     # load config
    cfg.merge_from_file('/mnt/TransT/pysot/core/config.py')
    # build attacker
    attacker = build_attacker('OIM', 'UA', max_num=10, reg_type='L21', eplison=3e-1)
    ...
    ##Initialize params for attack (UA)
    last_result = b1 = rect_from_mask(mask)
    prev_perts,weights = None,None
    ...
    while True:
        ...
        if not imagefile:
            break
        image = cv2.cvtColor(cv2.imread(imagefile), cv2.COLOR_BGR2RGB)  # Right
        b1, m, prev_perts, weights = tracker.track(image, attacker, prev_perts, weights, last_result)
        last_result = b1
        handle.report(m)
